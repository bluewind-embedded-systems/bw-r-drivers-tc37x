...
fn test_input_pin() {
    ...
    report.expect_read(P00.r#in().ptr(), 4, 0b00000000000000000000000000000000);
    let is_high = pin.is_high();
    assert!(!is_high);

    report.expect_read(P00.r#in().ptr(), 4, 0b00000000000000000000000010000000);
    let is_high = pin.is_high();
    assert!(is_high);
...
fn toggle_stateful_output_pin_stateful() {
    ...
    report.expect_read(P00.out().ptr(), 4, 0b00000000000000000000000000100000);
