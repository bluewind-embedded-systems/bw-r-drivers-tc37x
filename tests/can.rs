...
fn test_setup_can0() {
    ...
    // clear_cpu_endinit
    report.expect_read(
        SCU.wdtcpu()[0].wdtcpuycon0().ptr() as usize,
        4,
        0b11,
    );

    // clc read
    report.expect_read(CAN0.clc().ptr() as usize, 4, 0b0);
    ...
    report.expect_read(CAN0.mcr().ptr() as usize, 4, 0b0);
    report.expect_read(CAN0.mcr().ptr() as usize, 4, 0b11);

    // cccr for enable configuration change
    // read
    report.expect_read(CAN0.n()[0].cccri().ptr() as usize, 4, 0b1);
    ...
    // read
    report.expect_read(CAN0.n()[0].cccri().ptr() as usize, 4, 0b0);
    ...
    report.expect_read(CAN0.n()[0].tx().txesci().ptr() as usize, 4, 0b0);

    // txbc0 for set_tx_buffer_start_address for setup_tx
    report.expect_read(CAN0.n()[0].tx().txbci().ptr() as usize, 4, 0b0);

    // txbc0 for set_dedicated_tx_buffers_number for setup_tx
    report.expect_read(CAN0.n()[0].tx().txbci().ptr() as usize, 4, 0b100_0100_0000);

    // txbtie0 for enable_tx_buffer_transmission_interrupt for setup_tx
    report.expect_read(CAN0.n()[0].tx().txbtiei().ptr() as usize, 4, 0b0);
    report.expect_read(CAN0.n()[0].tx().txbtiei().ptr() as usize, 4, 0b1);

    // txefc0 for set_tx_event_fifo_start_address for setup_tx
    report.expect_read(CAN0.n()[0].tx().txefci().ptr() as usize, 4, 0b0);
    // txefc0 for set_tx_event_fifo_size for setup_tx
    report.expect_read(CAN0.n()[0].tx().txefci().ptr() as usize, 4, 0b100_0000_0000);

    // cccr0 for set_frame_mode for setup_tx
    report.expect_read(CAN0.n()[0].cccri().ptr() as usize, 4, 0b11);
    ...
    report.expect_read(CAN0.n()[0].rx().rxesci().ptr() as usize, 4, 0b0);

    // rxbc0 for set_rx_buffer_start_address for setup_rx
    report.expect_read(CAN0.n()[0].rx().rxbci().ptr() as usize, 4, 0b0);

    // rxesc0 for set_rx_fifo0_data_field_size for setup_rx
    report.expect_read(CAN0.n()[0].rx().rxesci().ptr() as usize, 4, 0b0);

    // rxf0c0 for set_rx_fifo0_start_address
    report.expect_read(CAN0.n()[0].rx().rxf0ci().ptr() as usize, 4, 0b0);

    // rxf0c0 for set_rx_fifo0_size
    report.expect_read(CAN0.n()[0].rx().rxf0ci().ptr() as usize, 4, 0b1_0000_0000);

    // rxf0c0 for set_rx_fifo0_operating_mode
    report.expect_read(
        CAN0.n()[0].rx().rxf0ci().ptr() as usize,
        4,
    ...
    report.expect_read(CAN0.n()[0].npcri().ptr() as usize, 4, 0b0);
    ...
    report.expect_read(CAN0.n()[0].grint2i().ptr() as usize, 4, 0b0);

    report.expect_read(SRC.can().can()[0].canxinty()[1].ptr() as usize, 4, 0b0);
    report.expect_read(SRC.can().can()[0].canxinty()[1].ptr() as usize, 4, 0b0);
    report.expect_read(SRC.can().can()[0].canxinty()[1].ptr() as usize, 4, 0b0);

    report.expect_read(CAN0.n()[0].iei().ptr() as usize, 4, 0b0);
